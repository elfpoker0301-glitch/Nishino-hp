...
// 管理画面連携: 実績情報を読み込み
function loadWorks() {
    const worksData = JSON.parse(localStorage.getItem('works') || '[]');
    const worksContainer = document.querySelector('.works-grid, .works-slider');
    
    if (!worksContainer || worksData.length === 0) return;
    ...
    if (worksContainer.classList.contains('works-slider')) {
        ...
        worksContainer.innerHTML = doubledWorks.map(work => `
